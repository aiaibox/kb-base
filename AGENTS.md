...
- has its own `scripts/lint.py`, an 8-line file that imports this repo's
  `lint.py` and calls `main(local=check)` with its own rules (or none)
- creates notes with `../base/scripts/newnote.sh <path> "<title>"` inside its work tree

The tools derive the target repo from `git rev-parse --show-toplevel` of the
working directory, never from where this file lives. Repo-specific rules are the
`check()` in each repo's `scripts/lint.py` — `public` bans private IPs and
internal hostnames; `private` requires numeric filenames and decrypts every
encrypted blob; `personal` and `business` have none.

## Rules for changing anything here

1. **A change here changes all four repos at once.** Run
   `for r in public personal business private; do (cd ../$r && python3 scripts/lint.py); done`
